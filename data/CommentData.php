...
//End select
    ...
    function selectidActivity($idActivity) {
        $query = $this->db->prepare("SELECT c.* ,a.* FROM tbcomment c INNER JOIN tbactor a on c.commentactor = a.actorid and c.commentstate = 1 WHERE c.commentactivityid =" . $idActivity . " ORDER BY(commentcoincidence);");
        $query->execute();
        $result = $query->fetchAll(PDO::FETCH_ASSOC);
        $query->closeCursor();
