...
class TagsData {
    ...
    function insert(Tag $tag) {
        $queryExistsMail = $this->db->prepare("SELECT tagword FROM tbtag WHERE tagactivityid=:activityid AND tagword=:word;");
        $queryExistsMail->execute(array('tagactivityid' => $tag->gettagactivityid(), 'word' => $tag->gettagword()));
